...
from lavis.datasets.datasets.camera_motion_cls_datasets import CameraMotionCLSDataset, CameraMotionCLSEvalDataset
from lavis.datasets.datasets.scoring_cls_datasets import ScoringCLSDataset, ScoringCLSEvalDataset
...
class BreakfastCLSBuilder(BaseDatasetBuilder):
    ...
    def build(self):
        ...
        for split in ann_info.keys():
            ...
            if not os.path.exists(vis_path):
                warnings.warn("storage path {} does not exists".format(vis_path))
...
@registry.register_builder("scoring_cls")
class ScoringCLSBuilder(BaseDatasetBuilder):
    train_dataset_cls = ScoringCLSDataset
    eval_dataset_cls = ScoringCLSEvalDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/scoring/defaults_cls.yaml",
    }
...
@registry.register_builder("camera_motion_cls")
class CameraMotionCLSBuilder(BaseDatasetBuilder):
    train_dataset_cls = CameraMotionCLSDataset
    eval_dataset_cls = CameraMotionCLSEvalDataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/camera_motion/defaults_cls.yaml",
        "oct_20_test_on_train": "configs/datasets/camera_motion/test.yaml",
    }
